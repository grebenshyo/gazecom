...
- Do NOT add Codex/OpenAI branding to git commits ("Generated with
  Codex", "Co-Authored-By: Codex", etc.).
- **Never commit without explicit user consent** — propose a message
  and wait.
- **Debugging / analysis requests are read-only by default.** Words like
  "pinpoint", "diagnose", "assess", "decide", "plan", or "what do you
  think" do not authorize file edits. Present findings and ask before
  changing files unless the user explicitly says to implement/change/write.
